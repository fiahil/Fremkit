...
pub trait Tx: Send + Sized + Clone + 'static {
    ...
    fn open(capacity: usize) -> (Self, Self::Receiver);
...
impl<T: Item> Tx for Arc<RwLock<Vec<T>>> {
    ...
    fn open(capacity: usize) -> (Self, Self::Receiver) {
        let v = Arc::new(RwLock::new(Vec::with_capacity(capacity)));
...
// LOG
//
impl<T: Item> Tx for Arc<Log<T>> {
    type Item = T;
    type Receiver = Arc<Log<T>>;

    fn open(capacity: usize) -> (Self, Self::Receiver) {
        let l = Arc::new(Log::new(capacity));

        (l.clone(), l)
    }

    fn write(&mut self, msg: Self::Item) {
        self.push(msg).expect("fremkit_channel write failed");
    }
}

impl<T: Item> Rx for Arc<Log<T>> {
    type Item = T;
...
fn single_thread_append<T: Tx>(b: &mut BenchmarkGroup<WallTime>, name: &str) {
    b.bench_function(name, |b| {
        b.iter_custom(|iters| {
            let (mut tx, _rx) = T::open(iters as usize);
...
fn multi_thread_append<T: Tx>(b: &mut BenchmarkGroup<WallTime>, name: &str, n_threads: usize) {
    b.bench_function(name, |b| {
        b.iter_custom(|iters| {
            let (tx, _rx) = T::open(iters as usize * n_threads);
...
fn multi_thread_read<T: Tx>(b: &mut BenchmarkGroup<WallTime>, name: &str, n_threads: usize) {
    b.bench_function(name, |b| {
        b.iter_custom(|iters| {
            let (tx, rx) = T::open(iters as usize * n_threads);
...
fn bench_single_thread_append(c: &mut Criterion) {
    let mut b = c.benchmark_group("bounded_single_thread_append");
    ...
    single_thread_append::<Arc<Log<u64>>>(&mut b, "my_log");

    b.finish();
}

fn bench_2_thread_append(c: &mut Criterion) {
    let mut b = c.benchmark_group("bounded_2_thread_append");
    ...
    multi_thread_append::<Arc<Log<u64>>>(&mut b, "my_log", 2);

    b.finish();
}

fn bench_8_thread_append(c: &mut Criterion) {
    let mut b = c.benchmark_group("bounded_8_thread_append");
